...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		$this->load->view('_templates/auth/_header.php', $this->data);
	...
	{
		$this->form_validation->set_rules('identity', str_replace(':', '', $this->lang->line('login_identity_label')), 'required|trim|valid_email');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($forgotten) {
				$config = [
					...
					'smtp_user' => 'bhatrix.auth@gmail.com',
					'smtp_pass' => 'bnsjpncpgetbaqgi',
					'smtp_port' => 465,
					'mailtype' => 'html',
					'charset' => 'utf-8',
					// 'newline' => "\r\n"
				...
				$this->email->from('bhatrix.auth@gmail.com');
				...
				if ($this->email->send()) {
					$this->session->set_flashdata('success', 'Email Send Succesfully');
					return redirect("auth/forgot_password", 'refresh');
				} else {
					echo "Email not send...";
